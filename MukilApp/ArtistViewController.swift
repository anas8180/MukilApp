...
class ArtistViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        manager.GET(
            kBaseURL+"feed_Artists",
            parameters: nil,
            success: { (operation: AFHTTPRequestOperation!,
                responseObject: AnyObject!) in
              //  println("JSON: " + responseObject.description)
                
                self.dataArray = responseObject.valueForKey("Our Artists") as! NSArray
    ...
    func configureCell(cell:CustomTableViewCell,indexPath:NSIndexPath)
    {
        ...
        cell.thumImage.layer.cornerRadius = cell.thumImage.frame.size.width/2
    ...
    */
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        let destinationView = segue.destinationViewController as! ArtistDetailViewController
        destinationView.profileID = self.dataArray.objectAtIndex(indexPath!.row).objectForKey("id") as! String
        destinationView.profileName = self.dataArray.objectAtIndex(indexPath!.row).objectForKey("name") as! String
    }
